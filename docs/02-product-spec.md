...
- 이관 계정의 비밀번호 변경 안내에서 `나중에 하기`를 누르면 현재 로그인 세션 동안만 숨기고, 다음 로그인 때 다시 안내합니다. 숫자 6자리로 변경하면 안내를 종료합니다.

### 내 예약 — 상단 메뉴에서 확인하는 학생 화면

로그인과 가입 직후에는 좌석 예약 화면을 먼저 보여줍니다. 상단 메뉴에서 언제든 ‘내 예약’에 접근할 수 있어야 합니다. 예약 성공 후에는 알림만 띄우고 새로고침하는 대신 새 예약 상세 또는 내 예약 화면으로 이동합니다.
...
| 최대 시간 | 기본 3시간, 관리자가 변경 가능 | 선택 즉시 초과 경고·확정 차단, 안내·서버 검증이 같은 값을 사용 |
| 학생별 제한 | 하루 활성 예약 1건 | 초기 유지 제안. 취소 후 재예약 허용 |
| 예약 시간 | 1시간 단위 시작·종료 선택 | 30분 단위 시작 시간과 이용 시간 선택, 종료 자동 계산 |
| 지난 시간 | 서버 제한 없음 | 과거 날짜와 이미 시작된 시간을 화면에서 즉시 안내하고 서버에서도 차단 |
| 예약 취소 | 시작 전 직접 취소 | 시작 전 또는 예약 생성 후 5분 이내만 학생이 직접 취소 |
| 중복 | 먼저 조회한 후 insert | 같은 좌석·시간에 DB 수준에서 한 예약만 확정 |
| 차단 | 날짜·시간 전체 차단 | 매주 정기 수업, 단발 전체 대관, 최대 60일간 여러 좌석 일괄 차단을 구분 |
...
- `ReservationSlot`: 예약 ID, 영업일, 좌석, 30분 단위 분 슬롯. `(date, seat_id, minute)` 유일성으로 점유 중복 방지. 취소 시 해당 점유만 해제하고 예약 이력은 유지.
...
- 관리자 화면의 `공지·설정`에서 예약 상단 공지와 시간 선택 아래의 이용 안내를 구분해 수정합니다.
...
- 예약 제출 시 접속 IP를 예약 기록에 저장하고 관리자에게만 표시. 학생 화면에 수집 목적을 안내.
